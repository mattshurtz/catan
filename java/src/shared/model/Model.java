...
public class Model {
	 ...
	 *             if the player list is empty, or if the index is invalid
	 */
	 ...
	 * in a valid location on the map, and if the player has a road left to play
	 *
	 * @param location
	 *            this is the location of the Road you would like to build
	 *            playerIndex identifies the player who would like to build the
	 *            road
	 * @return true if player has resources/pieces, false if the player
	 ...
	 * given EdgeLocation (Map.buildRoad())
	 *
	 * @param location
	 *            where the player is playing the road
	 */
	 ...
	 *
	 * @param location
	 *            where the player would like to build the settlement
	 * @return true if no exception is thrown
	 * @throws InsufficientSupplies
	 *             if the player did not have enough resources
	 * @throws InvalidLocation
	 *             if this is an invalid location for this player to play a
	 *             settlement
	 */
	...
	public boolean isValidRoadLocation(EdgeLocation location) {
		...
		for (Road road : map.getRoads()) {
			...
			if (normEdge.getDir() == EdgeDirection.North) {
				...
				if (road.getLocation().equals(new EdgeLocation(northeastNeighbor, EdgeDirection.NorthWest))
						&& road.getOwner() == currentPlayer) {
					return true;
				}
				HexLocation northwestNeighbor = normEdge.getHexLoc().getNeighborLoc(EdgeDirection.NorthWest);
				if (road.getLocation().equals(new EdgeLocation(northwestNeighbor, EdgeDirection.SouthEast))
						&& road.getOwner() == currentPlayer) {
					return true;
				}
				if (road.getLocation().equals(new EdgeLocation(normEdge.getHexLoc(), EdgeDirection.NorthEast))
						&& road.getOwner() == currentPlayer) {
					return true;
				}
				if (road.getLocation().equals(new EdgeLocation(normEdge.getHexLoc(), EdgeDirection.NorthWest))
						&& road.getOwner() == currentPlayer) {
					return true;
				}
			...
			if (normEdge.getDir() == EdgeDirection.NorthWest) {
				...
				HexLocation northwestNeighbor = normEdge.getHexLoc().getNeighborLoc(EdgeDirection.NorthWest);
				if (road.getLocation().equals(new EdgeLocation(northwestNeighbor, EdgeDirection.NorthEast))
						&& road.getOwner() == currentPlayer) {
					return true;
				}
				...
				if (road.getLocation().equals(new EdgeLocation(southwestNeighbor, EdgeDirection.NorthEast))
						&& road.getOwner() == currentPlayer) {
					return true;
				}
				if (road.getLocation().equals(new EdgeLocation(southwestNeighbor, EdgeDirection.North))
						&& road.getOwner() == currentPlayer) {
					return true;
				}
				if (road.getLocation().equals(new EdgeLocation(normEdge.getHexLoc(), EdgeDirection.North))
						&& road.getOwner() == currentPlayer) {
					return true;
				}
			}
			...
			if (normEdge.getDir() == EdgeDirection.NorthEast) {
				...
				if (road.getLocation().equals(new EdgeLocation(southeastNeighbor, EdgeDirection.NorthWest))
						&& road.getOwner() == currentPlayer) {
					return true;
				}
				if (road.getLocation().equals(new EdgeLocation(southeastNeighbor, EdgeDirection.North))
						&& road.getOwner() == currentPlayer) {
					return true;
				}
				if (road.getLocation().equals(new EdgeLocation(normEdge.getHexLoc(), EdgeDirection.North))
	...
	public boolean surroundingEdgeOfVertexHasRoad(VertexLocation location) {
		...
		int currentPlayer = turnTracker.getCurrentTurn();
		for (Road road : map.getRoads()) {
			// Assume that vObject.getLocation is returning the Normalized
			// location
			// If the hexDirection is northEast, check the current HexLocation's
			// NorthWest and East vertices for settlements, and the north
			// neighbor's east vertex
			// checks the surrounding vertices around the northeast vertex for
			// vertex objects
			if (normLocation.getDir() == VertexDirection.NorthEast) {
				...
				if (road.getLocation().equals(new EdgeLocation(normLocation.getHexLoc(), EdgeDirection.North))
						&& road.getOwner() == currentPlayer) {
					return true;
				}

				if (road.getLocation().equals(new EdgeLocation(normLocation.getHexLoc(), EdgeDirection.NorthEast))
						&& road.getOwner() == currentPlayer) {
					return true;
				}
			}
			// checks the surrounding vertices around the northwest vertex for
			// vertex objects
			if (normLocation.getDir() == VertexDirection.NorthWest) {
				...
				if (road.getLocation().equals(new EdgeLocation(normLocation.getHexLoc(), EdgeDirection.NorthWest))
						&& road.getOwner() == currentPlayer) {
					return true;
				}
			}
	...
	public boolean isValidVertex(VertexLocation location) {
		...
		for (VertexObject vObject : allVObjects) {
			...
			if (normVertLocation.getDir() == VertexDirection.NorthEast) {
				...
				if (vObject.getLocation().equals(new VertexLocation(northEastNeighbor, VertexDirection.NorthWest))) {
					return false;
				}
				// check southeast of current hex
				HexLocation southEastNeighbor = normVertLocation.getHexLoc().getNeighborLoc(EdgeDirection.SouthEast);

				if (vObject.getLocation().equals(new VertexLocation(southEastNeighbor, VertexDirection.NorthWest))) {
					return false;
				}

				if (vObject.getLocation().equals(
						new VertexLocation(normVertLocation.getHexLoc(), VertexDirection.NorthWest))) {
					return false;
				}
			}
			// checks the surrounding vertices around the northwest vertex for
			...
			if (normVertLocation.getDir() == VertexDirection.NorthWest) {
				...
				if (vObject.getLocation().equals(new VertexLocation(northwestNeighbor, VertexDirection.NorthEast))) {
					return false;
				}

				HexLocation southwestNeighbor = normVertLocation.getHexLoc().getNeighborLoc(EdgeDirection.SouthWest);

				if (vObject.getLocation().equals(new VertexLocation(southwestNeighbor, VertexDirection.NorthEast))) {
					return false;
				}

				if (vObject.getLocation().equals(
						new VertexLocation(normVertLocation.getHexLoc(), VertexDirection.NorthEast))) {
					return false;
				}
			}
		}
	 ...
	 * located at the given EdgeLocation (Map.buildSettlment())
	 *
	 * @param location
	 *            where the player is playing the road
	 * @param playerIndex
	 *            is used to identify the player playing the road
	 */
	 ...
	 *
	 * @param location
	 *            where the player would like to build the city
	 * @param playerIndex
	 *            identifies the player who would like to build this city
	...
	public boolean canBuildCity(VertexLocation location, int playerIndex) {
		...
		// Iterate through settlements to make sure the player owns a settlement
	 ...
	 * given VertexLocation with a city(Map.buildCity())
	 * 
	 * @param location
	 *            where the player would like to place a city
	 * @param playerIndex
	 *            used to identify the player building this settlement
	 */
